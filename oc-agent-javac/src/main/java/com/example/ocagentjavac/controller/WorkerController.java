...
public class WorkerController {
    ...
    private boolean isValidMainClass(String mainClassName, Path projectDir) throws IOException {
        ...
        // Удаляем расширение .java, если оно есть
        ...
        try (Stream<Path> paths = Files.walk(projectDir)) {
            return paths
                    ...
                    .anyMatch(path -> {
                        ...
                        return relativePath.endsWith(classPath);
                    });
        }
    }
    ...
    private ResponseEntity<?> handleMavenProject(Path projectDir, String mainClassName) throws IOException, InterruptedException {
        ...
        if (hasShadePlugin) {
            ...
            if (packageResult.getReturnCode() != 0) {
                List<Map<String, Object>> errors = parseMavenErrors(packageResult.getStderr(), projectDir);
                return buildErrorResponse(packageResult.getStdout(), errors, packageResult.getReturnCode());
            }
            ...
            if (compileResult.getReturnCode() != 0) {
                List<Map<String, Object>> errors = parseMavenErrors(compileResult.getStderr(), projectDir);
    ...
    private ResponseEntity<?> handleNonMavenProject(Path projectDir, String mainClassName) throws IOException, InterruptedException {
        ...
        javacCommand.add("-d");
        javacCommand.add(projectDir.toString());
        ...
        // Запуск главного класса
        List<String> javaRunCommand = Arrays.asList("java", "-cp", projectDir.toString(), mainClassName);
        CommandResult runResult = executeCommand(javaRunCommand, projectDir, 30);
        if (runResult.getReturnCode() != 0) {
            return buildErrorResponse(runResult.getStdout(), runResult.getStderr(), runResult.getReturnCode());
        }
    ...
    private List<Map<String, Object>> parseMavenErrors(String stderrOutput, Path projectDir) {
        List<Map<String, Object>> errorList = new ArrayList<>();
        ...
        String[] lines = stderrOutput.split("\n");
        Pattern errorPattern = Pattern.compile("^\\[ERROR\\] (.*\\.java):\\[(\\d+),(\\d+)\\] (.*)$");
        ...
        for (String line : lines) {
            ...
            if (matcher.matches()) {
                ...
                Map<String, Object> errorMap = new HashMap<>();
                errorMap.put("message", message);
                errorMap.put("file", "/" + Paths.get(filePath).getFileName().toString()); // Только имя файла
                errorMap.put("line", lineNumber);
                errorMap.put("column", columnNumber);
